...
public class Game {
    ...
    public void start(Player player1, Player player2) {
        ...
        outputMessage("Now to determine who goes first!");
        ...
        while (gameInPlay) {
            ...
            while(!turncomplete) {
                while (!filegiven) {
                    ...
                    if (action.length() >= 4 && action.substring(0, 4).equalsIgnoreCase("TEST")) {
                        String filename = action.length() > 4 ? action.substring(5).trim() : null;

                        if (filename == null || filename.isEmpty()) {
                            errorMessage("No filename provided. To test type : TEST <filename>");
                        } else {
                            commands = File.readFileToArray(filename);
                            if (commands != null) {
                                filegiven = true;
                            } else {
                        ...
                        if (processAction(action, currentPlayer, otherPlayer, otherPlayerNumber, false, null)) {
                ...
                if (!filegiven) {
                    ...
                    for (i = 0; i < commands.length && !turncomplete; i++) {
                        command = commands[i];
                        if (Objects.equals(command, "ROLL")) {
                            j = i + 1;
                            counter += 1;
                            while (commands[j].length() == 1 && counter == 1) {
                                move = commands[j];
                                if (!processAction(command, currentPlayer, otherPlayer, otherPlayerNumber, filegiven, move)) {
                                    break;
                                }
                                j++;
                            }
                            ...
                            if (processAction(command, currentPlayer, otherPlayer, otherPlayerNumber, false, null)) {
                                isPlayer1Turn = !isPlayer1Turn;
                                turncomplete = true;
                                break;
                            }
    ...
    private boolean processFile(String filename, Player player1, Player player2) {
        try(BufferedReader fileReader = new BufferedReader(new FileReader(filename))) {
            ...
            while ((command = fileReader.readLine()) != null) {
                ...
                outputMessage("Executing command from given file: " + command);

                if (command.equals("ROLL")) {
                    String moveCommand;
                    while ((moveCommand = fileReader.readLine()) != null && moveCommand.length() == 1) {
                        moveCommand = moveCommand.trim().toUpperCase();
                        System.out.println(moveCommand);
                        if (!processAction(command, currentPlayer, otherPlayer, otherPlayerNumber, usingFile, moveCommand)) {
                            return false;
                        }
                    }
                } else if (processAction(command, currentPlayer, otherPlayer, otherPlayerNumber, false, command)) {
                    isPlayer1Turn = !isPlayer1Turn;
                    currentPlayer = isPlayer1Turn ? player1 : player2;
                    otherPlayer = isPlayer1Turn ? player2 : player1;
                    otherPlayerNumber = isPlayer1Turn ? 2 : 1;
                }
            }
            return true;
     ...
     */
    private boolean processAction(String action, Player currentPlayer, Player otherPlayer, int otherPlayerNumber, boolean usingFile, String command) {
        ...
        switch (action.split(" ")[0]) {
            ...
            case "ROLL" -> {
                boolean play;
                play = handleRoll(currentPlayer, otherPlayer, usingFile, command);
                return play;
            }
    ...
    private void handleSeed(String action){
        try {
            long seed = Long.parseLong(action.substring(5).trim());
            setGameSeed(seed);
    ...
    public void setGameSeed(long seed) {
        dice.setSeed(seed); // Set the seed for dice rolls
        outputMessage("Game seed set to: " + seed);
    }

    private boolean handleRoll(Player currentPlayer, Player otherPlayer, boolean usingFile, String moveCommand) {
        switchPlayer = false;
        ...
        while (!switchPlayer) {
            ...
            // Display Legal Moves
            String[] moveParts = getUserMove(legalMoves, usingFile, moveCommand); // function to display the legal moves available and process user input
    ...
    private void pipCount() {
        ...
        for (int i = 0; i < 24; i++) {
            if (board.getPile(i).getColour() == Checker.Colour.WHITE) {
                noOfPipsWhite += board.getPile(i).getNoOfCheckers() * (i + 1);
            } else {
                noOfPipsBlack += board.getPile(i).getNoOfCheckers() * (24 - i);
            }
        }
        noOfPipsWhite += board.getBarPile(1).getNoOfCheckers() * (24);
        noOfPipsBlack += board.getBarPile(0).getNoOfCheckers() * (24);
    ...
    private String[] getUserMove(List<String> legalMoves, boolean usingFile, String moveCommand) {
        char moveLetter = 'A';
        ...
        while(true) {
            ...
            if (usingFile) {
                ...
                input = scanner.nextLine().toUpperCase(); // Convert input to uppercase for consistency
            }
            ...
            errorMessage("Invalid input. Please enter a letter corresponding to a legal move.");
        }
    }

    private void setDice(String action) {
        int[] diceValues = new int[2];
        try {
            String values  = action.substring(5).trim();
            ...
            if (isValidDicePair(parts)) {
                ...
                isDiceSet = true;
                outputMessage("Dice values now set for the next dice roll. Enter your next play or 'hint' for a list of possible commands: ");
            } else {
                errorMessage("Invalid input. Enter two valid integer values!");
            }
